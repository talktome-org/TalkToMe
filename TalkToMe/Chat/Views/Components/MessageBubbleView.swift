...
struct UserMessageBubbleView: View {
  ...
  @ViewBuilder
  private func attachmentsView(segments: [MessageSegment], alignment: HorizontalAlignment)
    -> some View
  {
  ...
  @ViewBuilder
  private func imageGrid(segments: [MessageSegment], size: CGFloat, cols: Int, isTrailing: Bool)
    -> some View
  {
...
extension View {
  fileprivate func chatImageOuterBorder(cornerRadius: CGFloat) -> some View {
    self.overlay(
...
struct MessageBubbleView: View {
  ...
  private var shouldShowVoiceRespondedLabel: Bool {
    ...
    if isLastAssistant {
      ...
      if chatViewModel.hasActiveStreamingCycle && !chatViewModel.hasSpokenCurrentResponse {
        return false
      }
    }
    return hasRenderableAssistantContent
  ...
  VStack(spacing: 20) {
    MessageBubbleView(
      ...
      message: ChatMessage.text(
        "Hello! How are you? I'm Stephan, and I'd like to chat with you.", isFromUser: true)
    )
    MessageBubbleView(
      chatViewModel: ChatViewModel(),
