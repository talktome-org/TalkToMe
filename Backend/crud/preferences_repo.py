...
async def get_daily_checkins_preference(*, user_id: uuid.UUID) -> Dict[str, Any]:
    ...
    return {"enabled": enabled, "hour": hour, "minute": minute, "timezone": timezone_val}
...
async def list_users_with_daily_checkins_enabled() -> List[Dict[str, Any]]:
    def _get_device_token_users():
        return supabase.table("device_tokens").select("user_id").eq("enabled", True).execute()
        ...
        users.append(
            {
                ...
                "daily_checkins_enabled": pref.get("daily_checkins_enabled", True),
