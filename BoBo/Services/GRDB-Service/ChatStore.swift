...
final class ChatStore {
    ...
    private func chatAttachmentsBaseDir() -> URL {
        ...
        let dir = appSupport.appendingPathComponent("BoBo/ChatAttachments/\(currentUserKey())", isDirectory: true)
        ...
        do {
            try await dbQueue.write { db in
                ...
                for dto in sortedDtos {
                    ...
                    // When server reconciliation replaces file:// URLs with remote URLs,
                    // immediately link existing local attachment files to the new remote URLs
                    // so that loadMessages can resolve them without waiting for a re-download.
                    if let existing, existing.content != dto.content {
                        let oldAtts = self.extractRemoteAttachments(messageId: dto.id.uuidString, content: existing.content)
                        let newAtts = self.extractRemoteAttachments(messageId: dto.id.uuidString, content: dto.content)
                        let hasLocalURLs = oldAtts.contains { URL(string: $0.downloadURL)?.isFileURL == true }
                        let hasRemoteURLs = newAtts.contains { url in
                            guard let u = URL(string: url.downloadURL) else { return false }
                            return u.scheme == "http" || u.scheme == "https"
                        }
                        if hasLocalURLs && hasRemoteURLs {
                            let localRecords = try ChatAttachmentRecord.fetchAll(
                                db,
                                sql: "SELECT * FROM attachments WHERE message_id = ? AND local_relpath IS NOT NULL",
                                arguments: [dto.id.uuidString]
                            )
                            for newAtt in newAtts {
                                guard let u = URL(string: newAtt.downloadURL), u.scheme == "http" || u.scheme == "https" else { continue }
                                let canonical = self.canonicalRemoteURL(newAtt.downloadURL)
                                let alreadyExists = try ChatAttachmentRecord.fetchOne(
                                    db,
                                    sql: "SELECT * FROM attachments WHERE message_id = ? AND remote_url = ?",
                                    arguments: [dto.id.uuidString, canonical]
                                )
                                if alreadyExists != nil { continue }
                                // Find a local record with matching kind to reuse
                                if let donor = localRecords.first(where: { $0.kind == newAtt.kind }) {
                                    let rec = ChatAttachmentRecord(
                                        pk: nil,
                                        message_id: dto.id.uuidString,
                                        kind: newAtt.kind,
                                        remote_url: canonical,
                                        local_relpath: donor.local_relpath,
                                        content_type: newAtt.contentType ?? donor.content_type,
                                        filename: newAtt.filename ?? donor.filename,
