...
extension ChatStore {
    ...
    func cacheOneAttachment(_ att: RemoteAttachment) async {
        ...
        let base: URL = {
            ...
            let dir = appSupport.appendingPathComponent("BoBo/ChatAttachments/\(key)", isDirectory: true)
        ...
        // Before downloading remotely, check if this message already has a cached
        // local file from the initial send (file:// URL record). This avoids a
        // redundant download and prevents the photo from briefly showing a loading
        // spinner when the server reconciliation replaces file:// URLs with remote ones.
        if url.scheme == "http" || url.scheme == "https" {
            do {
                let reused = try await dbQueue.write { db -> Bool in
                    let anyLocal = try ChatAttachmentRecord.fetchOne(
                        db,
                        sql: """
                        SELECT * FROM attachments
                        WHERE message_id = ? AND kind = ? AND local_relpath IS NOT NULL
                        LIMIT 1
                        """,
                        arguments: [att.messageId, att.kind]
                    )
                    guard let anyLocal, let rel = anyLocal.local_relpath else { return false }
                    let fileURL = base.appendingPathComponent(rel)
                    guard FileManager.default.fileExists(atPath: fileURL.path) else { return false }

                    // Create a record with the canonical remote URL pointing to the existing file
                    let rec = ChatAttachmentRecord(
                        ...
                        local_relpath: rel,
                        content_type: att.contentType ?? anyLocal.content_type,
                        filename: att.filename ?? anyLocal.filename,
                        created_at: anyLocal.created_at
                    )
                    try rec.save(db)
                    return true
                }
