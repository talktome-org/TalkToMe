...
final class DeepgramStreamingSTTService: ObservableObject, @unchecked Sendable {
    ...
    private let audioQueue = DispatchQueue(label: "bobo.deepgramSTT.audio")
    private let wsSendQueue = DispatchQueue(label: "bobo.deepgramSTT.wsSend")
    ...
    private func handle(jsonString: String) {
        ...
        if let t = dict["type"] as? String, t == "bobo.stt.ready" {
            return
        }
        if let t = dict["type"] as? String, t == "bobo.error" {
