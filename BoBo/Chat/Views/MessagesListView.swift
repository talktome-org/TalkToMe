    ...
    var body: some View {
        ...
        .onChange(of: chatViewModel.pendingOutgoingUserMessageId, initial: false) { _, newId in
            ...
            if let messageY = messagePositions[id] {
                ...
                // Use SwiftUI's scrollTo to force it to render; keep
                // pendingScrollTargetId so the onPreferenceChange handler
                // calls scrollToTopUIKit with enforcement as soon as the
                // position arrives (prevents SwiftUI re-renders from resetting
                // the scroll position before enforcement is active).
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
