...
class SettingsViewModel: ObservableObject {
    ...
    init() {
        ...
        if let cached = UserDefaults.standard.string(forKey: "bobo_profile_full_name"),
    ...
    func loadProfileInfo() {
        Task { @MainActor in
            do {
                ...
                self.isProfileLoaded = true
                UserDefaults.standard.set(self.fullName, forKey: "bobo_profile_full_name")
            } catch {
            ...
            if response.success {
                await MainActor.run {
                    ...
                    if self.fullName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                        UserDefaults.standard.removeObject(forKey: "bobo_profile_full_name")
                    } else {
                        UserDefaults.standard.set(self.fullName, forKey: "bobo_profile_full_name")
                    }
